...
# --- Configuration for Your Data Vault (Targeting 2 GB) ---
# Master, define your primary data storage location.
MAIN_DIR = r"C:\VaultOfForeverFiles"

# Number of main folders (e.g., C:\VaultOfForeverFiles\1, C:\VaultOfForeverFiles\2, etc.)
NUM_OUTER_FOLDERS = 1000

# Number of text files inside each main folder (e.g., C:\VaultOfForeverFiles\1\1.txt, 1\2.txt, etc.)
NUM_INNER_FILES_PER_FOLDER = 715 

# Number of "random words" (lines of random numbers) to put in each text file.
...
try:
    for outer_folder_idx in range(1, NUM_OUTER_FOLDERS + 1):
        ...
        for inner_file_idx in range(1, NUM_INNER_FILES_PER_FOLDER + 1):
            ...
            # --- Modification: Prepend "by P2X" to the content lines ---
            content_lines = ["by TR4"] + [str(random.randint(0, 32767)) for _ in range(RANDOM_WORDS_PER_FILE)]
